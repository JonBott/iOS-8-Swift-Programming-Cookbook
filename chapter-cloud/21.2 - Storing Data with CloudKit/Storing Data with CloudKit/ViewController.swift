...
class ViewController: UIViewController {
    ...
    enum CarType: String {
        ...
        func zoneId() -> CKRecordZoneID {
            let zoneId = CKRecordZoneID( zoneName: self.toRaw(), ownerName: CKOwnerDefaultName )
            return zoneId
        }
    ...
    func startiCloud() {
        ...
        if isIcloudAvailable() {
            displayAlertWithTitle("iCloud", message: "iCloud is not available. Please sign into your iCloud account and restart this app")
            return
        }
        ...
        database.fetchAllRecordZonesWithCompletionHandler{[weak self]
            ...
            if error != nil {
                ...
                self!.useOrSaveZone(zoneIsCreatedAlready: foundHatchbackZone, forCarType: .Hatchback)
    ...
    func useOrSaveZone(#zoneIsCreatedAlready: Bool, forCarType: CarType){
        ...
        if zoneIsCreatedAlready{
            ...
            println("Found the \(forCarType.toRaw()) zone. It's been created already")
            
            saveCarsForType(forCarType)
            
        } else {
            database.saveRecordZone(forCarType.zone(), completionHandler: {[weak self]
    ...
    {
        let fordFocus = hatchbackCarWithMaker("Ford", model: "Focus", numberOfDoors: 6, year: 2018)
    ...
    func saveEstateCars(){
        ...
        let volvoV50     = estateCarWithMaker("Volvo", model: "V50"    , numberOfDoors: 5, year: 2016)
        let audiA6       = estateCarWithMaker("Audi" , model: "A6"     , numberOfDoors: 5, year: 2018)
        let skodaOctavia = estateCarWithMaker("Skoda", model: "Octavia", numberOfDoors: 5, year: 2016)
    ...
    func hatchbackCarWithMaker(maker: String, model: String, numberOfDoors: Int, year: Int) -> CKRecord {
        return carWithType(.Hatchback, maker, model, numberOfDoors, year)
    }
    
    func estateCarWithMaker(maker: String, model: String, numberOfDoors: Int, year: Int) -> CKRecord {
        return carWithType(.Estate, maker, model, numberOfDoors, year)
    }
    
    func carWithType(type: CarType, _ maker: String, _ model: String, _ numberOfDoors: Int, _ year: Int) -> CKRecord {
